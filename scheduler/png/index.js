...
async function saveDummyFile(pngname,exportID){
	return new Promise(function(resolve, reject){
		fs.writeFile(pngname, "Connection closed:" + exportID, (error) => {
			if (error) throw error
			console.log(`Saved dummy file: ${pngname}`, new Date())
			resolve();
		})
	})
}
...
async function convert(data, user, mode, exportID) {
	...
	var pngname = tempFolder + (name || ("scheduler." + mode));
	...
	return fs.writeFileAsync(tempname, content)
		.then(async function() {
			...
			async function renderAndExport(){
				...
				let ps = spawn(electronBinPath, ["--ssl-protocol=any", "--ignore-ssl-errors=true", __dirname + '/electron-template.js', tempnamePath, pngname, JSON.stringify(additional_settings), pageRenderTimeout, "--no-sandbox", "--swiftshader", '--disable-gpu', '--disable-software-rasterizer', '--headless', "--disable-dev-shm-usage"]);
				...
				var stopExportTimerId = setInterval(async function(){
					...
					if (global.exportStack[exportID] == "finished"){
						...
						if (ps && ps.exitCode === null){
							...
							console.log(lastError)
							await saveDummyFile(pngname, exportID);
							ps.stdin.pause();
							ps.kill();
							ps = null;
						}
			...
			return (async function dummy(){})()
				.then(function() {
					if (fs.existsSync(pngname)) {
						return fs.readFileAsync(pngname);
					} else {
						if (lastError) {
